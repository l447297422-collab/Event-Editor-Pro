...
function handleNodeContextMenu(id) {
  ...
  if (!actionsProhibited) {
    if (idx >= 0) { // Event actions
      if (!classes.includes('fork') && !classes.includes('join')) {
        addAction(translations.edit_event || 'Edit event...', () => widget.editEvent(idx));
      }
      if (classes.includes('switch')) {
        addAction(translations.edit_cases || 'Edit cases...', () => widget.editSwitchBranches(idx));
      }
      if (classes.includes('fork')) {
        addAction(translations.edit_branches || 'Edit branches...', () => widget.editForkBranches(idx));
      }
      if (!classes.includes('join')) {
        actions.push({ divider: true });
      }

      addAction(translations.add_entry_point_here || 'Add entry point here...', () => widget.addEntryPoint(idx));
      actions.push({ divider: true });

      if (!classes.includes('join')) {
        addAction(translations.add_new_parent || 'Add new parent...', () => widget.addEventAbove(prevNodes, idx));
      }

      if (classes.includes('action') || classes.includes('sub_flow') || classes.includes('join')) {
        addAction(translations.add_new_child || 'Add new child...', () => widget.addEventBelow(idx));
        if (nextNodes.length) {
          addAction(translations.unlink_child || 'Unlink child', () => widget.unlink(idx));
        } else {
          addAction(translations.link_to_event || 'Link to event...', () => widget.link(idx));
        }
      }
      ...
      if (!isOnlyEventInEntry && (classes.includes('action') || classes.includes('sub_flow') || oneBranchSwitchOrFork)) {
        actions.push({ divider: true });
        addAction(translations.remove_event || 'Remove event', () => {
          isDeleting = true;
      ...
      addAction(translations.remove_entry_point || 'Remove entry point', () => widget.removeEntryPoint(idx));
    }
  ...
  if (graph.persistentWhitelist) {
    addAction(translations.show_all_events || 'Show all events', () => graph.renderOnlyConnected());
  } else {
    addAction(translations.show_only_connected_events || 'Show only connected events', () => graph.renderOnlyConnected(id));
  }
